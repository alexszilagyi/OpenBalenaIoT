...
class GoogleIoT {
  constructor (config = {}) {
    ...
    this.client = new iot.v1.DeviceManagerClient({ projectId: this.google.project, credentials: this.google.service })
  }
  ...
  async iotRegistration () {
    ...
    if (rsaCmd.code === 0) {
      let deviceObject = {
        id: this.device.id,
        credentials: [ { publicKey: { format: 'RSA-X509-PEM', key: fs.readFileSync(this.device.key.cert).toString() } } ]
      }
